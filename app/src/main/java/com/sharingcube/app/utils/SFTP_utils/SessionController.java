...
public class SessionController {
    ...
    public static boolean isConnected() {
        ...
        if (exists()) {
            ...
            Session mSession = getSessionController().getSession();
     ...
     */
    public void connect() {
        Log.i(TAG, "SessionController() connect() " + this.hashCode());
        if (mSession == null || (mSession != null && mSession.isConnected()) ) {
            mThread = new Thread(new SshRunnable());
            mThread.start();
            Log.i(TAG, "mSession = Null " + this.hashCode());
        } else if (!mSession.isConnected()) {
            Log.i(TAG, "mSession is not connected and mSession != null "  + this.hashCode());
            mThread = new Thread(new SshRunnable());
            mThread.start();
        }
    }
     ...
     */
    public void uploadFiles(File[] files, UploadFinishListener uploadFinishListener) {
        if (mSftpController == null) {
            mSftpController = new SftpController();

        }
        mSftpController.new UploadTask(mSession, files, uploadFinishListener, mConnectStatusListener).execute();
    }
     ...
     * @param srcPath
     * @param spm
     * @param transferMode: OVERRIDE-0, RESUME-1, APPEND-2
     * @return
     * @throws JSchException
     * @throws SftpException
     */
    public boolean downloadFile(String srcPath, String saveFilePath, SftpProgressMonitor spm, int transferMode) throws JSchException, SftpException {
        if (mSftpController == null) {
            mSftpController = new SftpController();
        }
        Log.d(TAG, "SrcPath: " + srcPath + ", saveFilePath: " + saveFilePath);
        mSftpController.new DownloadTask(mSession, srcPath, saveFilePath, spm, transferMode, mConnectStatusListener).execute();
        return true;
    }
    ...
    public void disconnect() throws IOException {
        ...
        boolean aa = mThread.isAlive();
        boolean bb = mThread != null;
    ...
    public class SshRunnable implements Runnable {
        ...
        public void run() {
            Log.i(TAG, "Start To Connect SFTP");
            JSch jsch = new JSch();
            mSession = null;
            try {
            ...
            if (mConnectStatusListener != null) {
                if (mSession.isConnected()) {
                    Log.i(TAG, "Connect SFTP Successfully");
                    mConnectStatusListener.onConnected();
                } else {
                    mConnectStatusListener.onConnectedFail();
                    Log.i(TAG, "Connect SFTP Fail");
                    // Do nothing
                }
            }
        }
